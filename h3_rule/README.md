...
| 라벨 | 파일 | 현재 인원 | 용도 |
|---|---|---:|---|
| `is_stagnant_cluster` | `data/cluster_labels.csv` | 412명 | 1단계 성장 정체 rule 학습 target |
| `is_current_parking_candidate` | `data/h1_current_candidates.csv` | 63명 | 최종 rule 평가 target |
| `is_high_confidence_candidate` | `data/h1_current_candidates.csv` | 35명 | 민감도 분석 target |

## 설계 원칙

- H1에서 직접 선택한 `cumexp_avg`, `union_avg`, `access_ratio`와 동일 신호를 그대로 복제하는 피처는 leakage 위험이 있으므로 주의한다.
