...
def smoke_test():
    ...
    if test and test.get("ranking"):
        ...
        return True
    print("  !! ranking/overall(전사-히어로, p1) 실패 — 위 [API ...] 메시지 확인\n")
    return False
        ...
        for s in stat.get("final_stat", []):
            if s.get("stat_name") == "전투력":
                try:
                    ...
                    # 일부 stat_value가 비정상 문자열이면 해당 날짜만 제외한다.
                    pass
...
def collect_group(group_name, seen_ocids, lock):
    ...
    print("     직업별: " + ", ".join(f"{c}:{n}" for c, n in sorted(nonzero.items(), key=lambda x: -x[1])))
    return group_rows
