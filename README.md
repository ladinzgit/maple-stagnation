...
- **핵심 설계 — 접속 피처를 분류기 입력에 포함**: 주차는 *성장 정체 + 활성 접속* 2차원 신호다. 휴면을 사전 필터로 제거하지 않고, 접속 피처(`access_active_months`, `access_ratio`, `access_recent`)를 **분류기 입력 피처로 넣어** 모델이 성장×접속 결합을 직접 학습하게 한다 (지도 학습 제약 유지).
- **방법**:
  1. Random Forest / XGBoost 학습 — pseudo-label = H1 현재 후보(`is_current_parking_candidate`, 성장 정체 ∩ 접속 활성), 5-fold stratified CV
  2. 입력 = 넓은 원시 관측 피처 + 접속 피처. **클러스터링 3피처(cumEXP·union·hexa_frag)는 순환 방지로 제외/제한**
  3. class imbalance: `class_weight='balanced'` / `scale_pos_weight`; positive n 부족 시 접속 임계 완화 라벨로 학습 → 엄격 라벨로 평가(2-tier)
  4. SHAP / permutation importance → 상위 2~3개 핵심 피처 선정 (접속·성장 상대 중요도 비교)
  5. 임계값 기반 단순 Rule 도출 (접속 항 포함, 예: `ΔcumEXP ≈ 0 AND Δunion ≈ 0 AND access_active_months >= 2`)
  6. Rule 단독 평가: Precision, Recall, F1, **FPR (오타겟팅률)**, ROC-AUC + Threshold sweep
- **수용 기준**: **Precision > 0.95 AND FPR < 5%**
- **전제 / 한계**: H1 수용 기준 충족 시 pseudo-label 신뢰도 확보. Ground truth 없음 → 지표는 pseudo-label 대비값, FPR은 휴면/일반 오분류율로 해석
...
| `access_active_months`, `access_active_weeks`, `access_ratio`, `access_recent` | 최근 로그인 활동 (`character/basic` `access_flag` 기반) — **H3 분류기 입력 피처**: 성장×접속 결합 학습용 |
